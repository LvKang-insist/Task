...
public class CalendarUtils {
    ...
    private static int checkCalendarAccounts(Context context) {
        ...
        try {
            userCursor = context.getContentResolver().query(Uri.parse(calanderURL), null, null, null, CalendarContract.Calendars.CALENDAR_ACCESS_LEVEL + " ASC ");
            if (userCursor == null) {
                return -1;
            }
    ...
    private static long addCalendarAccount(Context context) {
        ...
        try {
            result = context.getContentResolver().insert(calendarUri, value);
        } catch (Exception e) {
            e.printStackTrace();
        }
